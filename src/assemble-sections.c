...
static int
assemble_section(struct fw_section_entry *fw, const char *pfx, int file_fd)
{
	int fd;
	char str[128] = { 0, };
	uint32_t len;
	...
	printf("  importing 0x%08x -> 0x%08x (%d bytes)\n",
	    fw->start, fw->end, len);
...
main(int argc, const char *argv[])
{
	...
	if (argc < 3) {
		printf("usage: %s <output file> <input file prefix>\n",
		    argv[0]);
		exit(127);
	}
	fn = argv[1];
	...
	for (int i = 0; fw_sections[i].label != NULL; i++) {
		assemble_section(&fw_sections[i], fo, fd);
	}
